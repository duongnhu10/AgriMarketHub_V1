   ...
   <?php
    ...
    ?>
               <div class="food-menu-img">
                   <!-- Chọn sản phẩm cần  -->
                   <input type="checkbox" class="custom-checkbox" name="selected_items[]" value="<?php echo $id; ?>" onclick="updateSelection(this, <?php echo $id; ?>)">

                   <script>
                       // Hàm để cập nhật lựa chọn
                       function updateSelection(checkbox, itemId) {
                           // Kiểm tra trạng thái của checkbox (đã chọn hoặc không chọn)
                           var isSelected = checkbox.checked ? 1 : 0;

                           // Gửi yêu cầu AJAX
                           var xhr = new XMLHttpRequest();
                           xhr.open("POST", "update-selection.php", true);
                           ...
                           xhr.send("itemId=" + itemId + "&isSelected=" + isSelected);
                       }

                       // Hàm để đặt tất cả các hộp kiểm thành giá trị 0
                       function resetAllCheckboxes() {
                           // Lấy tất cả các hộp kiểm
                           var checkboxes = document.querySelectorAll('input[name="selected_items[]"]');

                           // Đặt trạng thái của tất cả các hộp kiểm về không chọn (0)
                           checkboxes.forEach(function(checkbox) {
                               checkbox.checked = false;
                               updateSelection(checkbox, checkbox.value); // Gọi hàm cập nhật lựa chọn với giá trị là 0
                           });
                       }

                       // Gọi hàm resetAllCheckboxes khi trang đã được tải hoàn toàn
               ...
               <div class="food-menu-desc">
                   ...
                   <script>
                       ...
                       function giamSoLuong(idSanPham) {
                           var inputSoLuong = document.getElementById("so_luong_" + idSanPham);
                           var soLuongHienTai = parseInt(inputSoLuong.value); //Số lượng hiện tại
                           var soLuongMoi = Math.max(soLuongHienTai - 1, 1); // Đảm bảo số lượng không âm
           ...
           <?php
            ...
            //Nút đặt hàng
            echo "<a href='" . SITEURL . "order-from-cart.php?session_user=" . $_SESSION['user'] . "' class='btn btn-secondary' onclick='kiemTraChonSanPham()'>Đặt hàng</a>";

            ?>
           <script>
               function kiemTraChonSanPham() {
                   // Lấy tất cả các hộp kiểm
                   var checkboxes = document.querySelectorAll('input[name="selected_items[]"]');
                   var checked = false;

                   // Kiểm tra từng hộp kiểm
                   checkboxes.forEach(function(checkbox) {
                       if (checkbox.checked) {
                           checked = true; // Đánh dấu là đã chọn ít nhất một sản phẩm
                       }
                   });

                   // Nếu không có sản phẩm nào được chọn, hiển thị thông báo và ngăn chặn mặc định của nút
                   if (!checked) {
